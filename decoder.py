...
allowed_properties = {
    'CONNECT': [17, 21, 22, 23, 25, 31, 33, 34, 38, 39],
    'CONNACK': [17, 18, 19, 21, 22, 26, 28, 31, 33, 34, 36, 37, 38, 39, 40, 41, 42],
    'PUBLISH': [1, 2, 3, 8, 9, 11, 31, 35, 38],
    'PUBACK': [31, 38],
    'PUBREC': [31, 38],
    'PUBREL': [31, 38],
    'PUBCOMP': [31, 38],
    ...
    'PINGREQ': [],
    'PINGRESP': [],
    'DISCONNECT': [17, 28, 31, 38],
    'AUTH': [21, 22, 31, 38],
    'WILL': [1, 2, 3, 8, 9, 24, 38],
...
class mqttdecode():
    ...
    def decode(self, hexstring):
        ...
        while self._packet is not None and len(self._packet) > 0:
            ...
            if self._packet is not None and len(self._packet) > 0:
                if self.args.no_separator == False:
                    print("------------------------------\n")
    ...
    def _get_output_v4(self):
        ...
        self._output_hex_v4 = self._output_hex.strip()
        self._output_text = ""
        self._output_hex = ""

    def _get_output_v5(self):
        self._output_text_v5 = self._output_text
        self._output_hex_v5 = self._output_hex.strip()
        self._output_text = ""
        self._output_hex = ""
    ...
    def _decode_varint(self, col, allowed_values=None):
        if self._rl <= 0:
        ...
        for i in range(0,4):
            ...
            try:
                byte = self._packet[0]
                self._packet = self._packet[1:]
                output_hex += f"{byte:02x}"
            except IndexError:
                varint = self.errstr
                self._output_hex += f"{self.cols['error']}{output_hex}{self.cols['reset']} "
                return (varint, vl)
            self._rl -= 1
        ...
        if allowed_values is not None and varint not in allowed_values:
            col = self.cols['error']
            pass

        self._output_hex += f"{col}{output_hex}{self.cols['reset']} "
        return (varint, vl)
    ...
    def _decode_properties(self, cmdname):
        if self._rl <= 0:
            return
        if self._packet is None or len(self._packet) == 0:
            return
        pl, _ = self._decode_varint(self.cols['length'])
        if pl > self._rl:
            self._output_text += f"Properties Length: {self.cols['error']}{pl} > remaining length{self.cols['reset']}\n"
            return
        else:
            self._output_text += f"Properties Length: {self.cols['length']}{pl}{self.cols['reset']}\n"
        if not isinstance(pl, int):
            return

        while self._rl > 0 and pl > 0:
            prop, vl = self._decode_varint(self.cols['proptype'], allowed_properties[cmdname])
            pl -= vl

            if prop in allowed_properties[cmdname]:
                propcol = self.cols['property']
                proptypecol = self.cols['proptype']
            else:
                propcol = self.cols['error']
                proptypecol = self.cols['error']
            ...
            try:
                ...
                return 0, None
            self._output_text += f"Property:          {proptypecol}{prop},{propdef[0]}{self.cols['reset']} = "
            if propdef[1] == "byte":
                v = self._decode_byte(propcol)
                pl -= 1
            elif propdef[1] == "uint16":
                v = self._decode_uint16(propcol)
                pl -= 2
            elif propdef[1] == "uint32":
                v = self._decode_uint32(propcol)
                pl -= 4
            elif propdef[1] == "varint":
                v, vl = self._decode_varint(propcol)
                pl -= vl
            elif propdef[1] == "string":
                v, _ = self._decode_string(propcol)
                pl -= (len(v) + 2)
            elif propdef[1] == "stringpair":
                k, _ = self._decode_string(propcol)
                v, _ = self._decode_string(propcol)
                pl -= (len(k) + 2 + len(v) + 2)
                self._output_text += f"{k}:"
            self._output_text += f"{propcol}{v}{self.cols['reset']}\n"
    ...
    def _decode_connect_v5(self, proto_name, rl_start):
        ...
        self._decode_properties('CONNECT')
        self._decode_excess_bytes()
    ...
    def _decode_connack_v5(self):
        ...
        try:
            ...
            rc_str = f"{self.cols['error']}Invalid reason code{self.cols['reset']}"
        self._output_text += f"Reason code:       {self.cols['rc']}{rc} ({rc_str}){self.cols['reset']}\n"
        self._decode_properties('CONNACK')
        self._decode_excess_bytes()
        self._get_output_v5()
    ...
    def _decode_publish_v5(self):
        ...
        if qos == 1 or qos == 2:
            self._decode_mid()
        self._decode_properties('PUBLISH')
        self._decode_payload()
        self._get_output_v5()
    ...
    def _decode_pubflow_v5(self, cmdname):
        self._restore_state()
        self._output_hex = f"{self.cols['command']}{self._hdr:02x}{self.cols['reset']} " + self._output_hex
        self._output_text += f"Command:           {self.cols['command']}{cmdname} v5.0{self.cols['reset']}\n"
        self._print_fixed_header(0x00)
        self._print_length(self._rl)
        self._decode_mid()
        if self._packet is not None and len(self._packet) > 0:
        ...
        self._decode_properties(cmdname)
        self._decode_excess_bytes()
        self._get_output_v5()


    def _decode_pubflow_v4(self, cmdname):
        self._output_hex = f"{self.cols['command']}{self._hdr:02x}{self.cols['reset']} " + self._output_hex
        self._output_text += f"Command:           {self.cols['command']}{cmdname} v3.1.1{self.cols['reset']}\n"
        self._print_fixed_header(0x00)
        self._print_length(self._rl)
        self._decode_mid()
        self._decode_excess_bytes()
    ...
    def _decode_puback(self):
        if self.args.force_spec is None or self.args.force_spec == 4:
            self._decode_pubflow_v4('PUBACK')

        if self.args.force_spec is None or self.args.force_spec == 5:
            self._decode_pubflow_v5('PUBACK')


    def _decode_pubrec(self):
        if self.args.force_spec is None or self.args.force_spec == 4:
            self._decode_pubflow_v4('PUBREC')

        if self.args.force_spec is None or self.args.force_spec == 5:
            self._decode_pubflow_v5('PUBREC')


    def _decode_pubrel(self):
        if self.args.force_spec is None or self.args.force_spec == 4:
            self._decode_pubflow_v4('PUBREL')

        if self.args.force_spec is None or self.args.force_spec == 5:
            self._decode_pubflow_v5('PUBREL')


    def _decode_pubcomp(self):
        if self.args.force_spec is None or self.args.force_spec == 4:
            self._decode_pubflow_v4('PUBCOMP')

        if self.args.force_spec is None or self.args.force_spec == 5:
            self._decode_pubflow_v5('PUBCOMP')
    ...
    def _decode_subscribe_v5(self):
        ...
        self._print_length(self._rl)
        self._decode_mid()
        self._decode_properties('SUBSCRIBE')
        while self._rl > 0:
            topic, topic_len = self._decode_string(self.cols['topic'])
    ...
    def _decode_suback_v5(self):
        ...
        self._print_length(self._rl)
        self._decode_mid()
        self._decode_properties('SUBACK')
        if self._packet is None or len(self._packet) == 0:
    ...
    def _decode_unsubscribe_v5(self):
        ...
        self._print_length(self._rl)
        self._decode_mid()
        self._decode_properties('UNSUBSCRIBE')
        while self._rl > 0:
            topic, topic_len = self._decode_string(self.cols['topic'])
            self._output_text += f"Topic:             ({self.cols['strlen']}length: {topic_len}{self.cols['reset']}) {self.cols['topic']}{topic}{self.cols['reset']}\n"
    ...
    def _decode_unsuback_v5(self):
        ...
        self._print_length(self._rl)
        self._decode_mid()
        self._decode_properties('UNSUBACK')
        while self._rl > 0:
            self._decode_reason_code()
        self._get_output_v5()
    ...
    def _decode_disconnect_v5(self):
        ...
        self._print_length(self._rl)
        self._decode_reason_code()
        self._decode_properties('DISCONNECT')
        self._decode_excess_bytes()
        self._get_output_v5()
    ...
    def _decode_packet(self):
        ...
        if isinstance(self._rl, str):
            self._output_hex_v4 = f"{self.cols['error']}{self._hdr:02x}{self.cols['reset']}"
            return
...
def get_args():
    ...
    parser.add_argument(
        ...
        help="treat data as binary rather than hex. Expects --file"
    )

    group.add_argument(
        "--colour", action="store_true", default=None,
        help="colourise the output"
    )
    group.add_argument(
        "--no-colour", action="store_true", default=None,
        help="do not colourise the output"
    )

    parser.add_argument(
        '--no-separator', action="store_true", default=False,
        help="don't print the ------- separator between different packets"
    )
...
def check_args(args):
    if args.no_colour == True:
        args.colour = False

    if args.colour is None:
        # Default to colour if a TTY and colour not explicitly set, otherwise turn off
